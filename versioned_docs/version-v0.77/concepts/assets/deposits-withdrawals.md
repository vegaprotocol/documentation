...
To use assets, they need to be deposited: 
...
* After approving, choose an amount to deposit. This deposit will have to accepted on the Ethereum network and the given network before the assets can be used

Vega software has built in support for ERC-20 assets. An asset's ERC-20 token contract needs to be available on one of the bridge before it can be used, which happens in the governance and enactment process for new assets.
...
To remove assets from a network, submit a withdrawal request via an app or using [Etherscan](../../tutorials/assets-tokens/withdrawing-assets.md). 
...
If it's a successful withdrawal transaction, the bridge will emit an `Asset_Withdrawn` event, and confirm to the relevant network that the withdrawal has been completed.
...
### Withdrawal limits
Withdrawals can have limits associated with them, where trying to withdraw above a certain amount will cause that withdrawal to be delayed by a set time. If a validator is compromised or otherwise issues a bad withdrawal, the delay gives the network operators a chance to stop the withdrawal before it's too late.
...
:::tip Query for data
**[Use the get asset API](../../api/rest/data-v2/trading-data-service-get-asset.api.mdx)** to see the withdrawal threshold.
:::

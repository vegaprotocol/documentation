...
A debug tool exists that allows the contents of a snapshot for a particular block to be written to a json file. This can contain useful information that can help to understand problems with the network and something a member of the development team may ask for to help debug an issue.

...
import {
  NetworkParameterContext,
  fetchData
} from './lib/NetworkParameterStore'
  ...
  if (data) {
    ...
    if (props.formatter === 'governanceToken') {
      if (value === '1' || value === 1) {
        ...
        formattedValue =
          props.formatter && formatters[props.formatter]
            ? formatters[props.formatter](value)
            : value
      }
    } else {
      formattedValue =
    ...
    if (suffix) {
      ...
      if (
        !skipSuffixFix &&
        suffix === 'tokens' &&
        (value === '1' || formattedValue === 1 || formattedValue === '1')
      ) {
      ...
      displayValue = (
        <strong>
          {formattedValue} {suffixCorrected}
        </strong>
      )
    } else {
    ...
    return (
      <a
        href={`${explorerUrl[vega_network]}#${props.param}`}
      ...
      >
        ...
        {hideValue
          ? null
          : (
            <span className='networkparametervalue'>
              {displayValue || `Could not find ${props.param}`}
            </span>
            )}
      </a>
    )

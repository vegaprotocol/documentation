...
function etherscanLink(contractAddress, network){
    ...
    return <a href={`${base}${contractAddress}`} target="_blank" rel="noreferrer">🔍</a>
}
...
const showOnlyDefault = ['vegaTokenAddress', 'vestingAddress', 'stakingBridge', 'erc20Bridge']
...
export default function EthAddresses({ frontMatter, show }) {
    ...
    return (
      <table>
          ...
          <tbody>
              {Object.keys(addresses).filter(key => addresses[key].length > 8 && showOnly.indexOf(key) !== -1).map(key => {
                  return <tr>
                      <td><strong>{contractNames[key]}</strong></td>
                      <td><code>{addresses[key]}</code> {etherscanLink(addresses[key], ethereum_network)}</td>

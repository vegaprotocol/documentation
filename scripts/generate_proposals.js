...
if (!process.env.VEGA_VERSION) {
  console.error(
    "Please set an environment variable VEGA_VERSION (e.g. VEGA_VERSION=v0.55.0"
  );
  process.exit(1);
}
...
function output(partial, title) {
  ...
  if (process.argv[2]) {
    ...
    if (title === "newMarket") {
      ...
      writeFileSync(
        `${path}/_${title}_json_instrument.md`,
        partial.excerpts.instrument
      );
      writeFileSync(
        `${path}/_${title}_json_overview.md`,
        partial.excerpts.overview
      );
      writeFileSync(
        `${path}/_${title}_json_liqparams.md`,
        partial.excerpts.liqparams
      );
      writeFileSync(

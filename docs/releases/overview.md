...
Version 0.70.1 contains the fixes and minor enhancements to verify in testnet before the validators deploy for the Market Simulation #4.
...
**Rename table `current liquidity provisions` to `live liquiditiy provisions` and add a `live` option** During testing it was identified that over time the current liquidity provisions table will continue to grow as LPs are created/deleted. This change will help the management of the data being created by the protocol.

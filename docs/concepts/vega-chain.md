...
Validator candidates that have submitted a transaction to become consensus validator nodes will need to send a hash of block number `b`, separately signed by the three required keys and submitted, during each epoch and every set number of blocks (`numBlocks`). 
...
1. Start a Vega validator node, including the associated [infrastructure](../node-operators/requirements/infrastructure.md)
...
4. Announce the node to community and wait for tokenholders to nominate the node

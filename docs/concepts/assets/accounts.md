...
### Holding accounts
Holding accounts, like settlement accounts, temporarily holds assets to be distributed based on the outcomes of trades and market movements. However, holding accounts are specific to spot markets.

An order on a spot market that doesn't immediately trade in full, has the collateral needed to cover the order, and its fees, moved to a holding account, to ensure you can support your open order. If you cancel or reduce the size, any freed up collateral is returned to your general account. This is true in both continuous trading and auction modes.

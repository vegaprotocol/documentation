...
1. Create the new version folder in the `<VEGA-VISOR-HOME>`, e.g., for version `v0.72.14`, run the following command: `mkdir -p <VEGA-VISOR-HOME>/v0.72.14`.
2. Download the new version of the Vega binary from the [releases page ↗](https://github.com/vegaprotocol/vega/releases)
3. Unzip the downloaded binary into the created directory, e.g. `<VEGA-VISOR-HOME>/v0.72.14/vega` binary
4. Create the run configuration and put it in the created directory, e.g. `<VEGA-VISOR-HOME>/v0.72.14/run-config.toml` run config file (see example below)

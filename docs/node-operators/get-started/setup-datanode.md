...
Data node software currently supports Postgres 16 and TimescaleDB 2.15.3.
...
Find the PostgreSQL parameters in the `postgresq.conf` file. The default file path for Linux and PostgreSQL 16 is: `/etc/postgresql/16/main/postgresql.conf`.
...
```
vega datanode init --home=$YOUR_DATANODE_HOME_PATH "insert-chain-id-here"
```
...
4. Find the list of network history bootstrap nodes by querying the network history bootstrap API: `/api/v2/networkhistory/bootstrap`
...
```toml
  [NetworkHistory]
    [NetworkHistory.Store]
      BootstrapPeers = ["insert-network-peers-here"]
```
...
The data node will by default pull all the entire chain's history and is expected to take almost 24 hours. The `--networkhistory.initialise.block-count` option can be used to limit the amount of data pulled, but this is not recommended for the purposes of a public data node.
:::
...
Make sure you disable statesync by setting `statesync.enabled = false` in the CometBFT configuration and `AutoInitialiseFromNetworkHistory = false` in the network history. Otherwise your node will FAIL after next restart.
:::

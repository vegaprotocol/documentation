...
By default the data node will retain all historic data generated by the chain. This is the expected behaviour for public data nodes. Find out more about data retention see the section on [data node retention policies](#data-node-retention-profiles).
...
The data node will by default pull all the entire chain's history and is expected to take almost 24 hours. The `--networkhistory.initialise.block-count` option can be
used to limit the amount of data pulled, but this is not recommended for the purposed of a public data node.
:::

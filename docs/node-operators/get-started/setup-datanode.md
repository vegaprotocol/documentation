...
The default PostgreSQL configuration is not optimized for memory usage. We have to modify a few PostgreSQL parameters in the `postgresq.conf` file. The default parameter file path for Linux system and PostgreSQL 14 is: `/etc/postgresql/14/main/postgresql.conf`.

Suggested parameters for the `PostgreSQL` are:
...
We should limit the maximum number of connections as it directly impacts the memory usage by PostgreSQL.
...
The default value of the `huge_pages` config is `try`. Setting it to `off` usually reduce the RAM usage, but it increases the CPU usage on the machine.
...
New value depends on your server memory. It should be set to `25%` of the system physical memory. The `2GB` value should be for the server with `8GB` physical memory.
...
So We can assume that `Max RAM` utilization may be rounded to: `shared_buffer + (temp_buffers + work_mem) * max_connections`.

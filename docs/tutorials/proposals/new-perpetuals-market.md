...
The mark price methodology can be fine-tuned per market. If left blank, the market will default to the [last price method](../../concepts/trading-on-vega/margin.md#last-traded-price).
...
| `cashAmount` | Used in calculating the mark price from the order book, in asset decimals. | If you expect the market to be highly liquid, use a value the equivalent to 200 USD. If you expect this market to have 10x less liquidity than it would on a high-volume platform, use the USD 20 equivalent. |
| `sourceWeights` | Determines how much weight goes to each composite price component. The order of sources used is as follows: price by trades, price by book, oracle_1, ... oracle_n, median price. | 0, 1, 0 |

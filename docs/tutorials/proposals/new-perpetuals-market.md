...
| `tickSize` | Sets the minimal change in the price in the market. The tick size needs to be specified in terms of the market decimals, as an integer multiple of 10^mdp. | If the BTCUSDT market is configured with 5 mdp, using tick size 1 would allow the smallest tick size of 0.00001. To deal with having too many decimal places, you can set the tick size to 100000 (which converts to 0.1). Should an asset's value drop dramatically, the tick size can then be changed. | A scaled tick size of `0.02` in a market using `5` decimal places would mean a tick size of `2000`. |
